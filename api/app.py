...
def _commit(job: dict, candidate: Candidate, evidence: dict, operator: str):
    try:
        ...
        chain = run_node_json("contracts/scripts/writeRecord.ts", storage["rootHash"], job["photo_hash"],
                              url_commitment, job["directory"] / "chain_transaction.json", timeout=240)
        job["chain"] = chain
        _checkpoint(job, "chain_mined")
        result_for_index = {
            "run_id": job["id"], "storage": storage, "chain": chain,
            "readback_verified": False,
        }
        save_index(DATABASE, f"web-upload:{job['id']}", record, result_for_index)
        from verify_record import verify
        readback = verify(str(job["photo_path"]), chain["recordId"], chain["contractAddress"],
                          record_path, candidate.page_url)
